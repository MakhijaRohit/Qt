...
CityBlock::CityBlock(Kind kind)
{
    ...
    int red     = 96 + (std::rand() % 64);
    int blue   = 96 + (std::rand() % 64);
    ...
    else if(kind == SmallBuilding)
    {
        ...
        block.moveBottomLeft(QPointF((std::rand() % 6) -3, (std::rand() % 6) -3));
        shape.addRect(block);
    }
    ...
    else if (kind == Hall)
    {
        ...
        shape.addEllipse(boundingRect().adjusted(+padding1, +padding1, -padding2, -padding2));
    }
    ...
    else if (kind == BlockPlusTwoSmallBlocks)
    {
        ...
        shape.addRect(QRectF(-16 + w1 + 4, y + h2 + 4 + (std::rand() % 4), w3, h3));
    }
}
